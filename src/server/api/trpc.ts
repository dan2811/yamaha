...
const getRoleBasedProcedures = () => {
  const procedures = {} as Record<
    `${Role}Procedure`,
    typeof protectedProcedure
  >;
  roles.forEach((role, index) => {
    const procedureKey = `${role}Procedure` as const;
    procedures[procedureKey] = protectedProcedure.use(({ ctx, next }) => {
